...
void mult_vec_shared_x(int n, int rows_per_thread, float *y, float *d, float *A,
  ...
  for (int i = 0; i < copies_per_thread; i++) {
    int copy_index = (((threadIdx.y * blockDim.x) + threadIdx.x) * copies_per_thread) + i;//Change this line based on the responsibility of this thread to fetch part of x
  ...
  for (int i = 0; i < rows_per_thread; i++) {
    ...
    double sum = d[row_index];
    for (int j = 0; j < n; j++) {
      sum += A[row_index*n + j]*shared_x[j];
    }
    y[row_index] = sum;

    diff[row_index] = fabs(sum - shared_x[row_index]);
  }
...
int it_mult_vec(int N,
  ...
  cudaMemcpy(y, y_d, row_size, cudaMemcpyDeviceToHost);
